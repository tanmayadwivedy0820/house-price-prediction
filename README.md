...
# \- \*\*Pipeline:\*\* median imputation + standard scaling for numeric features; most-frequent imputation + one-hot encoding for `ocean\_proximity`; all baked into a single scikit-learn `Pipeline` so the API can send raw values.
...
# \- \*\*Top features (permutation importance):\*\* `median\_income`, then geographic position (`longitude`/`latitude`) and `ocean\_proximity` — consistent with how California housing prices actually behave.
...
# \# copy .env.example to .env and set PROJECT\_ROOT to the absolute backend path
